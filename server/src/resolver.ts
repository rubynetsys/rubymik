...
import { buildBlockyConfig, probeBlockedDomain, type ResolverSettings } from './dnsfilter.js';
...
export interface ReloadResult { ok: boolean; detail: string; }
...
export async function reloadAndVerify(opts: {
  ...
  probeHost: string; probePort: number; settings: ResolverSettings;
  ...
  catch (e) { return { ok: false, detail: `Could not restart the resolver (${(e as Error).message}). Is the filtering profile deployed?` }; }
  if (code >= 300) return { ok: false, detail: `The resolver restart was rejected (Docker API ${code}).` };

  // wait for it to come back up (answering any control query)
  let up = false;
  for (let i = 0; i < 20; i++) {
    try { await dnsQuery(opts.probeHost, opts.probePort, 'cloudflare.com'); up = true; break; } catch { await sleep(1000); }
  }
  if (!up) return { ok: false, detail: 'The resolver did not answer within 20s after reload — NOT reporting the change as applied. Check the resolver logs.' };

  const blockDomain = probeBlockedDomain(opts.settings);
  if (!blockDomain) return { ok: true, detail: 'Applied — the resolver is up and answering (no reliable block-probe for this rule set).' };
  let ips: string[];
  try { ips = await dnsQuery(opts.probeHost, opts.probePort, blockDomain); }
  catch (e) { return { ok: false, detail: `Reloaded, but the block-probe query failed: ${(e as Error).message}.` }; }
  const blocked = ips.length === 0 || ips.every((x) => x === '0.0.0.0' || x === '::');
  if (!blocked) return { ok: false, detail: `Reloaded, but "${blockDomain}" is NOT being blocked (got ${ips.join(', ')}) — the rules may not have taken.` };
  return { ok: true, detail: `Applied and verified — the resolver is up and blocking "${blockDomain}".` };
}
