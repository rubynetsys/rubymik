...
export default function Onboard() {
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <header>
        <h1 className="flex items-center gap-2 text-2xl font-bold text-fg-strong"><Wand2 className="h-6 w-6 text-accent" /> Onboard a router</h1>
        <p className="mt-1 text-sm text-fg-dim">Bring an existing, live MikroTik under RubyMIK management — same-LAN or behind NAT.</p>
      </header>
      ...
      {!path ? (
        ...
        <div className="rounded-2xl border border-border bg-surface p-6">
          ...
          <div className="mt-6 flex items-center justify-between">
            <button onClick={() => (step === 0 ? reset() : go(-1))} className="inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-sm font-semibold text-fg-muted hover:bg-app">
            ...
            {cur === 'Finish' ? (
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg bg-accent px-5 py-2 text-sm font-semibold text-inverse hover:bg-accent-hover disabled:opacity-40"
              >
...
function DoNoHarmBanner() {
  return (
    <div className="flex items-start gap-3 rounded-xl border border-success-line bg-success-bg/60 px-4 py-3">
...
function PathChooser({ hub, onPick }: { hub: RemoteAccessView['hub'] | null; onPick: (p: Path) => void }) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <button onClick={() => onPick('direct')} className="rounded-2xl border border-border bg-surface p-6 text-left transition hover:border-accent-border hover:shadow-sm">
        <Network className="h-8 w-8 text-accent" />
        <h3 className="mt-3 font-bold text-fg-strong">Router on my network</h3>
        <p className="mt-1 text-sm text-fg-dim">Reachable directly on the LAN RubyMIK is on. Pure monitoring attach — no change to the router.</p>
      </button>
      <button onClick={() => onPick('tunnel')} className="rounded-2xl border border-border bg-surface p-6 text-left transition hover:border-accent-border hover:shadow-sm">
        <RadioTower className="h-8 w-8 text-accent" />
        <h3 className="mt-3 font-bold text-fg-strong">Router at a remote site (behind NAT)</h3>
        <p className="mt-1 text-sm text-fg-dim">Not directly reachable. It dials an outbound WireGuard tunnel into RubyMIK (the only additive change).</p>
        {!tunnelReady && (
          <span className="mt-3 inline-flex items-center gap-1 rounded-full bg-warning-bg px-2 py-0.5 text-xs font-semibold text-warning-fg">
    ...
    <ol className="mb-6 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs font-semibold">
      {steps.map((s, i) => (
        <li key={s} className="flex items-center gap-2">
          <span className={`flex h-6 w-6 items-center justify-center rounded-full ${i < step ? 'bg-success-strong text-inverse' : i === step ? 'bg-accent text-inverse' : 'bg-border text-fg-dim'}`}>
            {i < step ? <CheckCircle2 className="h-4 w-4" /> : i + 1}
          </span>
          <span className={i === step ? 'text-fg-strong' : 'text-fg-faint'}>{s}</span>
          {i < steps.length - 1 && <span className="mx-1 text-fg-faint">›</span>}
        </li>
      ))}
    </ol>
  );
...
function TunnelStep(props: {
  ...
  if (!hub?.enabled || !hub?.running) {
    return (
      <div className="rounded-xl bg-warning-bg p-4 text-sm text-warning-fg">
      ...
      {!peerId ? (
        <div>
          ...
          <label className="mt-3 block">
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Name for this router</span>
            <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="Cape Town branch gateway" autoFocus />
          </label>
          <button onClick={() => void provision()} disabled={busy} className="mt-4 inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-inverse hover:bg-accent-hover disabled:opacity-50">{busy && <Loader2 className="h-4 w-4 animate-spin" />} Generate bootstrap</button>
        </div>
      ) : (
        <>
          ...
          <div>
            ...
            <div className="mt-2 flex items-end gap-2">
              ...
              <button onClick={() => void register()} disabled={busy || !pubkey.trim() || handshake} className="inline-flex items-center gap-2 rounded-lg bg-accent px-4 py-2 text-sm font-semibold text-inverse hover:bg-accent-hover disabled:opacity-50">{busy && <Loader2 className="h-4 w-4 animate-spin" />} Register</button>
            </div>
          </div>
          <div className={`flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold ${handshake ? 'bg-success-bg text-success-fg' : 'bg-app text-fg-muted'}`}>
...
function AdditiveBootstrap({ tunnelIp, bootstrap }: { tunnelIp: string; bootstrap: string }) {
  return (
    <div className="rounded-xl border border-border p-4">
      ...
      <div className="mt-3 rounded-lg bg-success-bg/70 p-3 text-sm text-success-fg">
        <div className="font-semibold">This only ADDS a management tunnel. It changes nothing else on your router.</div>
        <ul className="mt-1.5 list-disc space-y-0.5 pl-5 text-success-fg">
          <li>a WireGuard interface <code className="rounded bg-success-bg px-1">rmik-wg</code> (the router generates its own private key)</li>
          <li>the overlay address <code className="rounded bg-success-bg px-1">{tunnelIp}</code> on that interface</li>
    ...
    <div className="space-y-4">
      ...
      {path === 'direct' && (
        <label className="block">
          <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Router IP / hostname</span>
          <input className={inputCls} value={host} onChange={(e) => setHost(e.target.value)} placeholder="192.168.88.1" autoFocus />
        </label>
      )}
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="block"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Read username</span><input className={inputCls} value={readUser} onChange={(e) => setReadUser(e.target.value)} placeholder="rubymik-ro" /></label>
        <label className="block"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Read password</span><input className={inputCls} type="password" value={readPass} onChange={(e) => setReadPass(e.target.value)} /></label>
      </div>
      <button onClick={() => void identify()} disabled={busy || !effectiveHost || !readUser} className="inline-flex items-center gap-2 rounded-lg bg-fg px-4 py-2 text-sm font-semibold text-inverse hover:bg-fg-body disabled:opacity-50">
        {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Eye className="h-4 w-4" />} Test & identify
      </button>
      {info && (
        <div className="rounded-xl border border-success-line bg-success-bg/50 p-4">
          <div className="flex items-center gap-2 text-sm font-bold text-success-fg"><CheckCircle2 className="h-4 w-4" /> Reached the router — is this the right one?</div>
          ...
          <label className="mt-3 block">
            <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Name in RubyMIK</span>
...
function ManagementStep({ wantWrite, setWantWrite, writeUser, setWriteUser, writePass, setWritePass }: { wantWrite: boolean; setWantWrite: (b: boolean) => void; writeUser: string; setWriteUser: (s: string) => void; writePass: string; setWritePass: (s: string) => void }) {
  return (
    <div className="space-y-4">
      ...
      <label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${!wantWrite ? 'border-accent-border bg-accent-subtle/40' : 'border-border'}`}>
        <input type="radio" checked={!wantWrite} onChange={() => setWantWrite(false)} className="mt-1 h-4 w-4 accent-accent" />
        <div><div className="flex items-center gap-1.5 font-semibold text-fg"><Eye className="h-4 w-4" /> Monitor only</div><div className="text-sm text-fg-dim">RubyMIK can never change this router. Zero write capability.</div></div>
      </label>
      <label className={`flex cursor-pointer items-start gap-3 rounded-xl border p-4 ${wantWrite ? 'border-accent-border bg-accent-subtle/40' : 'border-border'}`}>
        <input type="radio" checked={wantWrite} onChange={() => setWantWrite(true)} className="mt-1 h-4 w-4 accent-accent" />
        <div className="flex-1">
          <div className="flex items-center gap-1.5 font-semibold text-fg"><Lock className="h-4 w-4" /> Also allow management (add a write credential)</div>
          <div className="text-sm text-fg-dim">Enables config features later. Still writes nothing during onboarding.</div>
          {wantWrite && (
...
function SiteStep({ sites, siteId, setSiteId, newSiteName, setNewSiteName }: { sites: Site[]; siteId: number | ''; setSiteId: (v: number | '') => void; newSiteName: string; setNewSiteName: (s: string) => void }) {
  return (
    <div className="space-y-4">
      ...
      <label className="block">
        <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-fg-dim">Create a new site</span>
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4">
        <input type="checkbox" checked={backupNow} onChange={(e) => setBackupNow(e.target.checked)} className="mt-1 h-4 w-4 accent-accent" />
        <div><div className="flex items-center gap-1.5 font-semibold text-fg"><Archive className="h-4 w-4" /> Take an initial backup now <span className="rounded-full bg-info-bg px-2 py-0.5 text-[11px] font-semibold text-info-fg">recommended</span></div>
          <div className="text-sm text-fg-dim">A read-only {manageable ? 'export' : 'snapshot'} captured as a day-one restore point. Writes nothing to the router.</div></div>
      </label>
      <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-border p-4">
        <input type="checkbox" checked={scheduled} onChange={(e) => setScheduled(e.target.checked)} className="mt-1 h-4 w-4 accent-accent" />
        <div><div className="flex items-center gap-1.5 font-semibold text-fg"><Clock className="h-4 w-4" /> Include in automatic scheduled backups</div>
          <div className="text-sm text-fg-dim">Periodic read-only config backups on the fleet schedule. Off unless you opt in here.</div></div>
      </label>
    </div>
  );
}
...
function ReviewStep({ path, name, host, tunnelIp, info, wantWrite, siteName, backupNow, scheduled }: { path: Path; name: string; host: string; tunnelIp: string; info: DeviceInfo | null; wantWrite: boolean; siteName: string | null; backupNow: boolean; scheduled: boolean }) {
  return (
    <div className="space-y-4">
      ...
      <dl className="grid grid-cols-2 gap-x-6 gap-y-2 rounded-xl bg-sunken p-4 text-sm sm:grid-cols-3">
...
function FinishButton(props: {
  ...
  return (
    <button onClick={() => void finish()} disabled={p.busy} className="inline-flex items-center gap-2 rounded-lg bg-success px-6 py-2 text-sm font-semibold text-inverse hover:bg-success-strong disabled:opacity-50">
...
function DoneSummary({ path, result, wantWrite, onDone, onAnother }: { path: Path; result: { deviceId: number; siteName: string | null; backupTaken: boolean }; wantWrite: boolean; onDone: () => void; onAnother: () => void }) {
  return (
    <div className="mx-auto max-w-2xl">
      <div className="rounded-2xl border border-success-line bg-surface p-8 text-center">
        <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-success-bg"><CheckCircle2 className="h-8 w-8 text-success" /></div>
        <h2 className="mt-4 text-xl font-bold text-fg-strong">RubyMIK is now monitoring this router.</h2>
        ...
        <div className="mt-6 flex justify-center gap-3">
          <button onClick={onDone} className="rounded-lg bg-accent px-5 py-2 text-sm font-semibold text-inverse hover:bg-accent-hover">Open device</button>
          <button onClick={onAnother} className="rounded-lg border border-border-strong px-5 py-2 text-sm font-semibold text-fg-body hover:bg-sunken">Onboard another</button>
        </div>
      </div>
    </div>
  );
...
function StepTitle({ icon: Icon, title, sub }: { icon: React.ComponentType<{ className?: string }>; title: string; sub: string }) {
  return (
    <div className="flex items-start gap-2.5">
      ...
      <div><h3 className="font-bold text-fg-strong">{title}</h3><p className="text-sm text-fg-dim">{sub}</p></div>
    </div>
  );
}
function Meta({ label, value }: { label: string; value: string }) {
  return <div className="min-w-0"><dt className="text-[11px] font-semibold uppercase tracking-wide text-fg-faint">{label}</dt><dd className="mt-0.5 truncate text-sm font-medium text-fg" title={value}>{value}</dd></div>;
}
function Line({ label, ok, neutral }: { label: string; ok?: boolean; neutral?: boolean }) {
  const Icon = neutral ? RadioTower : ok ? CheckCircle2 : X;
  return <div className="flex items-start gap-2"><Icon className={`mt-0.5 h-4 w-4 shrink-0 ${neutral ? 'text-info' : 'text-success'}`} /><span className="text-fg-body">{label}</span></div>;
}
