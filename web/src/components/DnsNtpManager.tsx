...
export default function DnsNtpManager({ deviceId }: { deviceId: number }) {
  ...
  if (error && !view) return <div className="rounded-lg bg-red-50 px-3 py-2.5 text-sm text-red-800">Could not load DNS/NTP: {error}</div>;
  ...
  return (
    <div className="space-y-6">
      {ro && (
        <div className="flex items-center gap-2 rounded-lg bg-zinc-100 px-3 py-2 text-xs font-medium text-zinc-600">
...
function DnsPanel({ view, ro, deviceId, onOutcome, reload }: { view: NetConfigView; ro: boolean; deviceId: number; onOutcome: SetOutcome; reload: () => Promise<void> }) {
  ...
  return (
    <div>
      <h3 className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-zinc-500"><Globe className="h-3.5 w-3.5" /> DNS</h3>
        ...
        <div className="rounded-xl border border-zinc-200 p-3.5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="block sm:col-span-2">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">DNS servers (comma-sep IPs)</span>
              <input className={inputCls} value={servers} onChange={(e) => setServers(e.target.value)} placeholder="1.1.1.1, 8.8.8.8" />
            </label>
            <label className="block">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">Cache (KiB)</span>
              <input className={inputCls} value={cache} onChange={(e) => setCache(e.target.value)} inputMode="numeric" />
            </label>
          </div>
          <label className="mt-3 flex items-start gap-2 text-sm">
            <input type="checkbox" checked={allow} onChange={(e) => setAllow(e.target.checked)} className="mt-0.5 h-4 w-4 accent-ruby-600" />
            <span><span className="font-medium text-zinc-800">Allow remote requests</span>
              <span className="block text-xs text-zinc-500">Makes the router a DNS resolver for its clients. Useful — but don't enable blindly (exposes the resolver to that network).</span>
            </span>
          </label>
          <div className="mt-3 flex justify-end">
            <button onClick={() => void apply()} disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-ruby-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-ruby-500 disabled:opacity-50">
      ...
      <div className="mt-3">
        <div className="mb-1.5 flex items-center justify-between">
          <span className="text-[11px] font-bold uppercase tracking-wide text-zinc-500">Static entries · {dns.static.length}</span>
          {!ro && <button onClick={() => setAddStatic(true)} className="inline-flex items-center gap-1 text-xs font-semibold text-ruby-700 hover:underline"><Plus className="h-3.5 w-3.5" /> Add</button>}
        </div>
        {dns.static.length === 0 ? (
          <div className="rounded-lg bg-zinc-50 px-3 py-2 text-sm text-zinc-500">No static DNS entries.</div>
        ) : (
          <div className="overflow-hidden rounded-lg border border-zinc-100">
            {dns.static.map((e) => (
              <div key={e.id} className="flex items-center gap-3 border-b border-zinc-50 px-3 py-1.5 text-sm last:border-0">
                <span className="font-medium text-zinc-800">{e.name}</span>
                <span className="font-mono text-xs text-zinc-500">{e.address}</span>
                {e.comment && <span className="text-xs text-zinc-400">{e.comment}</span>}
                {!ro && <button onClick={() => void removeStatic(e.id, `${e.name} → ${e.address}`)} className="ml-auto rounded-md p-1.5 text-zinc-400 hover:bg-red-50 hover:text-red-700"><Trash2 className="h-4 w-4" /></button>}
              </div>
            ))}
          </div>
        )}
...
function NtpPanel({ ntp, ro, deviceId, onOutcome, reload }: { ntp: NtpState; ro: boolean; deviceId: number; onOutcome: SetOutcome; reload: () => Promise<void> }) {
  ...
  const statusCls = live.synced ? 'bg-emerald-50 text-emerald-700' : live.enabled ? 'bg-amber-50 text-amber-700' : 'bg-zinc-100 text-zinc-500';
  return (
    <div>
      <h3 className="mb-2 flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-zinc-500"><Timer className="h-3.5 w-3.5" /> NTP (time sync)</h3>
      <dl className="mb-3 grid grid-cols-2 gap-x-6 gap-y-2 text-sm sm:grid-cols-4">
        <div>
      ...
      {!ro && (
        <div className="rounded-xl border border-zinc-200 p-3.5">
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            <label className="block sm:col-span-2">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">NTP servers (IP or hostname)</span>
            ...
            <label className="flex items-center gap-2 pt-5 text-sm font-medium text-zinc-700">
              <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} className="h-4 w-4 accent-ruby-600" /> Enable NTP client
            </label>
          </div>
          <div className="mt-3 flex justify-end">
            <button onClick={() => void apply()} disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-ruby-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-ruby-500 disabled:opacity-50">
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-950/60 p-4" onMouseDown={onClose}>
      <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl" onMouseDown={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between"><h3 className="text-lg font-bold text-zinc-900">Static DNS entry</h3><button onClick={onClose} className="rounded-lg p-1.5 text-zinc-400 hover:bg-zinc-100"><X className="h-5 w-5" /></button></div>
        <p className="mt-1 text-xs text-zinc-500">A hostname → IP mapping the router resolves locally. Goes through the audited pipeline.</p>
        <div className="mt-4 space-y-3">
          {err && <div className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-800">{err}</div>}
          <label className="block"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">Hostname</span><input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="nas.lan" autoFocus /></label>
          <label className="block"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">IP address</span><input className={inputCls} value={address} onChange={(e) => setAddress(e.target.value)} placeholder="192.168.90.10" /></label>
          <label className="block"><span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-zinc-500">Comment (optional)</span><input className={inputCls} value={comment} onChange={(e) => setComment(e.target.value)} /></label>
        </div>
        <div className="mt-5 flex justify-end gap-3">
          ...
          <button onClick={() => void save()} disabled={busy} className="inline-flex items-center gap-2 rounded-lg bg-ruby-600 px-5 py-2 text-sm font-semibold text-white hover:bg-ruby-500 disabled:opacity-50">{busy && <Loader2 className="h-4 w-4 animate-spin" />}Add entry</button>
        </div>
      </div>
    </div>
  );
